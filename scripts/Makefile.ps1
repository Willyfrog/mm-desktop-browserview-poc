...
function Backup-ComputerState {
    $env:COM_DPOC_MAKEFILE_PATH_BACKUP = $env:Path
...
function Restore-ComputerState {
    ...
    # Remove all COM_DPOC_MAKEFILE_ prefixed env variable
    foreach ($item in (Get-Item -Path Env:*)) {
        if ($item.Name -imatch 'COM_DPOC_MAKEFILE_') {
            Print-Info "Removing Dpoc env variable: $($item.Name)..."
...
function Run-BuildId {
    ...
    $env:COM_DPOC_MAKEFILE_BUILD_ID = "$version"
    Print " [$env:COM_DPOC_MAKEFILE_BUILD_ID]"

    Print-Info -NoNewLine "Getting build id version for msi..."
    $env:COM_DPOC_MAKEFILE_BUILD_ID_MSI = $winVersion.Split('.')[0..3] -Join '.'
    Print " [$env:COM_DPOC_MAKEFILE_BUILD_ID_MSI]"

    Print-Info -NoNewLine "Getting build id version for node/npm..."
    $env:COM_DPOC_MAKEFILE_BUILD_ID_NODE = $version
    Print " [$env:COM_DPOC_MAKEFILE_BUILD_ID_NODE]"
    ...
    $msiDescriptor.Wix.Product.Version = [string]$env:COM_DPOC_MAKEFILE_BUILD_ID_MSI
    $ComponentDownload = $msiDescriptor.CreateElement("Property", "http://schemas.microsoft.com/wix/2006/wi")
    $ComponentDownload.InnerText = "https://releases.dpoc.com/desktop/$version/dpoc-desktop-$version-`$(var.Platform).msi"
...
function Run-BuildChangelog {
    ...
    $env:COM_DPOC_MAKEFILE_BUILD_CHANGELOG = $changelog
}
...
function Run-BuildForceSignature {
    ...
    if ($env:APPVEYOR_REPO_TAG -eq $true) {
        ...
        # .\resources\windows\certificate\dpoc-desktop-windows.pfx
        iex ((New-Object Net.WebClient).DownloadString(
        ...
        foreach ($archPath in "dist\win-unpacked", "dist\win-ia32-unpacked") {
            ...
            Get-ChildItem -Path $archPath -recurse "*.dll" | ForEach-Object {
                ...
                Start-Sleep -s 15
                signtool.exe sign /f "resources\windows\certificate\dpoc-desktop-windows.pfx" /p "$env:COM_DPOC_MAKEFILE_CERTIFICATE_PRIVATE_KEY_ENCRYPTED" /tr "http://timestamp.digicert.com" /fd sha1 /td sha1 "$($_.FullName)"
                Start-Sleep -s 15
                signtool.exe sign /f "resources\windows\certificate\dpoc-desktop-windows.pfx" /p "$env:COM_DPOC_MAKEFILE_CERTIFICATE_PRIVATE_KEY_ENCRYPTED" /tr "http://timestamp.digicert.com" /fd sha256 /td sha256 /as "$($_.FullName)"
            }
        ...
        foreach ($archPath in "dist\win-unpacked", "dist\win-ia32-unpacked") {
            ...
            Get-ChildItem -Path $archPath -recurse "*.dll" | ForEach-Object {
                ...
                # src.: https://web.archive.org/web/20190306223053/https://github.com/electron-userland/electron-builder/issues/2795#issuecomment-466831315
                Start-Sleep -s 15
                signtool.exe sign /f "./dpoc-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha1 /td sha1 "$($_.FullName)"
                Start-Sleep -s 15
                signtool.exe sign /f "./dpoc-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha256 /td sha256 /as "$($_.FullName)"
            }
...
function Get-Cert {
    if (Test-Path 'env:PFX') {
        ...
        [IO.File]::WriteAllBytes("./dpoc-desktop-windows.pfx", [Convert]::FromBase64String($env:PFX))
        $password = "$env:PFX_KEY" | convertto-securestring -asplaintext -force
        Print-Info "Importing certificate into the machine"
        Import-PfxCertificate -filepath "./dpoc-desktop-windows.pfx" cert:\localMachine\my -password $password
    } else {
...
function Remove-Cert {
    if (Test-Path 'env:PFX') {
        ...
        Remove-Item -path "./dpoc-desktop-windows.pfx"
    }
}
...
function Main {
    try {
        ...
        $env:COM_DPOC_MAKEFILE_EXECUTION_SUCCESS = $true
        $exitcode = 0

    } catch {
        switch ($_.Exception.Message) {
            "com.dpoc.makefile.deps.missing" {
            ...
            "com.dpoc.makefile.deps.notadmin" {
        ...
        if (!($env:COM_DPOC_MAKEFILE_EXECUTION_SUCCESS)) {
